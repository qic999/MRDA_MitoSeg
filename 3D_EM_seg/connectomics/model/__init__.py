...
def build_model(cfg, device, checkpoint=None):
    ...
    if cfg.MODEL.ARCHITECTURE == 'fpn_3d':
    ...
    elif cfg.MODEL.ARCHITECTURE == 'unet_plus_3d':
        model = MODEL_MAP[cfg.MODEL.ARCHITECTURE](**kwargs)
    elif cfg.MODEL.ARCHITECTURE == 'unet_3d':
        model = MODEL_MAP[cfg.MODEL.ARCHITECTURE]()
    elif cfg.MODEL.ARCHITECTURE == 'vnet_3d':
        model = MODEL_MAP[cfg.MODEL.ARCHITECTURE]()
...
def build_monitor(cfg):
    ...
    log_path = os.path.join(cfg.DATASET.LOG_PATH, 'log_'+cfg.SOLVER.TRAIN_NAME, 'train')
