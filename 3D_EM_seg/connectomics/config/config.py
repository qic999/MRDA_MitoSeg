...
# Padding mode, possible options: 'zeros','circular', 'replicate'
...
_C.DATASET.TRAIN_DO_CHUNK_TITLE = 0
_C.DATASET.TEST_DO_CHUNK_TITLE = 0
_C.DATASET.TRAIN_SingleVolume = 0
_C.DATASET.LOG_PATH = ''
_C.DATASET.EPFL = 0
_C.DATASET.MitoH = 0
_C.DATASET.MitoR = 0
_C.DATASET.Kasthuri = 0
...
_C.SOLVER.TEST_BEGAIN_TESP = 50000
_C.SOLVER.EPOCH_PERIOD = 1000
...
def save_all_cfg(cfg, output_dir):
    ...
    path = os.path.join(cfg.DATASET.LOG_PATH, 'log_'+cfg.SOLVER.TRAIN_NAME, "config.yaml")
    if not os.path.exists(os.path.join(cfg.DATASET.LOG_PATH, 'log_'+cfg.SOLVER.TRAIN_NAME)):
        os.makedirs(os.path.join(cfg.DATASET.LOG_PATH, 'log_'+cfg.SOLVER.TRAIN_NAME))
