...
from connectomics.utils.evaluation.vol3d_eval import VOL3Deval
from connectomics.utils.evaluation.vol3d_util import seg_iou3d_sorted, readh5_handle, readh5, unique_chunk
...
def get_args_eval_mito():
    ...
    parser.add_argument('-external_input', action='store_true')
...
def load_data(predict_seg, gt_seg, args, slices):
    # load data arguments
    pred_seg = readh5_handle(predict_seg)
    gt_seg = readh5_handle(gt_seg)
    print(predict_seg)
    print(gt_seg)
    if slices[1] == -1:
    ...
    if args.predict_score != '':
        ...
        pred_score[:, 0] = ui
        pred_score[:, 1] = uc
        # print('load data pred_score',pred_score)
...
def eval_mito(gt_seg, predict_seg, output_name):
    """
    ...
    print('\t1. Load data')
    args = get_args_eval_mito()
    if args.external_input:
        output_name = args.output_name
        gt_seg = args.gt_seg
        predict_seg = args.predict_seg
    ...
    gt_seg, pred_seg, pred_score, areaRng, slices = load_data(predict_seg, gt_seg, args, slices)


    model_num = int(predict_seg.split("/")[-1][:6]) # 000000 取出来
    path = ("/").join(predict_seg.split("/")[:-1]) # 提取save model 路径下的model
    ...
    # print('process pred_score',pred_score)  # process pred_score []
    ...
    if output_name == '':
        output_name = predict_seg[:predict_seg.rfind('.')]
    v3dEval = VOL3Deval(result_p, result_fn, pred_score_sorted, model_num, path, output_name=output_name)
    if args.do_txt > 0:
...
if __name__ == '__main__':
    ...
    predict_seg = ''
    output_name = ''
    eval_mito(gt_seg, predict_seg, output_name)
