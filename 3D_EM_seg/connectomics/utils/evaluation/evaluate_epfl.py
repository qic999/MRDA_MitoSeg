...
def get_args_eval_epfl():
    parser = argparse.ArgumentParser(prog='eval_epfl',
        description='iou')
    parser.add_argument('-gt_semantic', type=str)
    parser.add_argument('-gt_instance', type=str)
    parser.add_argument('-p_semantic', type=str)
    parser.add_argument('-p_instance', type=str)
    parser.add_argument('-external_input', action='store_true')
    parser.add_argument('-o', default="outputs/inference_output", type=str) # output
    parser.add_argument('--config-file', type=str, help='configuration file (yaml)')

    args_eval_epfl = parser.parse_args()

    return args_eval_epfl
...
def metric(seg_img, lab):
    ...
    if not aji_valid_num ==0 :
        ...
        final_aji = 0
    final_jac = (jac_all / 165)
    final_dice = (dice_all / 165)
    print("jac:", final_jac)
...
def eval_epfl(gt_instance, p_instance, output_txt):
    args = get_args_eval_epfl()
    if args.external_input:
        output_txt = args.o
        gt_instance = args.gt_instance
        p_instance = args.p_instance
...
if __name__ == '__main__':
    ...
    p_instance = ''
    output_txt = ''
    eval_epfl(gt_instance, p_instance, output_txt)