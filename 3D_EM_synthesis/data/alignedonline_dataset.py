...
class AlignedonlineDataset(BaseDataset):
    def __init__(self,opt,
        ...
        if os.path.isdir(self.volume_path) and not len(os.listdir(self.volume_path)) == 1:
            self.volume = [io.volread(os.path.join(self.volume_path, i)) for i in os.listdir(self.volume_path)]
            self.label = [io.volread(os.path.join(self.label_path, i)) for i in os.listdir(self.label_path)]
        else:
        ...
        if opt.aug:
            ...
            self.augmentor = build_train_augmentor(opt)
            sample_volume_size = self.augmentor.sample_size
            sample_label_size = self.augmentor.sample_size
    ...
    def __getitem__(self, index):
        ...
        out_volume = np.expand_dims(out_volume, 0).astype(dtype=np.float32)
        out_label = np.expand_dims(out_label, 0).astype(dtype=np.float32)
        # print(out_label.shape)
        # print(out_volume.shape)

        A = torch.from_numpy(out_label).to(dtype=torch.float)
        B = torch.from_numpy(out_volume).to(dtype=torch.float)
